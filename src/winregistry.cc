...
namespace
{
  ...
  napi_value SetStringRegKey(napi_env env, napi_callback_info info)
  {
    ...
    if (value.length() + 1 > MAXDWORD)
    {
      napi_throw_error(env, nullptr, "Value too long for registry");
      return nullptr;
    }

    if (ERROR_SUCCESS != RegSetValueEx(hKey, name.c_str(), 0, REG_SZ, (BYTE *)value.c_str(),
  ...
  napi_value DeleteRegKey(napi_env env, napi_callback_info info)
  {
    napi_value argv[3];
    size_t argc = 3;
  ...
  napi_value Init(napi_env env, napi_value exports)
  {
    ...
    napi_value deleteRegKey;
    napi_create_function(env, "DeleteRegKey", NAPI_AUTO_LENGTH,
                         DeleteRegKey, NULL, &deleteRegKey);
    napi_set_named_property(env, exports, "DeleteRegKey", deleteRegKey);
